...
def test_format_issue_with_row_and_multiple_columns():
    ...
    assert (
        result
        == "[WARNING] data.csv: Row 10, Columns [age, dob] have conflicting values"
    )
...
def test_format_issue_without_rows_and_columns():
    ...
    result = Utils.format_issue(issue)
    assert result == "[ERROR] file.csv: , Columns [header1, header2] No rows found"
...
def test_format_issue_without_row_with_column():
    ...
    result = Utils.format_issue(issue)
    assert (
        result == "[ERROR] schema.json: , Columns [header1, header2] Duplicate columns"
    )
